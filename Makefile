...
docker-build:
	...
	docker build -t  565122144984.dkr.ecr.us-east-1.amazonaws.com/analytics-service:$(image_tag) .
	docker push 565122144984.dkr.ecr.us-east-1.amazonaws.com/analytics-service:$(image_tag)

eks-deploy:
	git pull
	aws eks update-kubeconfig --name wmp-dev
	helm upgrade -i analytics-service helm -f helm/values/analytics-service.yaml --set image_tag=$(image_tag)
...
argocd-deploy:
	...
	argocd app create analytics-service \
		...
		--sync-policy auto \
		--upsert \
		--values values/analytics-service.yaml \
